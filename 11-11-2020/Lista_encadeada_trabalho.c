...
void buscar_por_nome(){
	...
	printf("\n");
	while(al_atual_aux!=NULL){
	if (strstr(al_atual_aux->nome, nome)!=NULL){ //A função strstr devolve um ponteiro para a primeira ocorrência da string apontada por endereçoStrChave na string apontada por endereçoStrOrigem
				printf("%s\n",al_atual_aux->nome);
				reg_enc++;
...
void excluir_todos(){
	...
	struct Aluno *al_atual_aux = al_atual;
				...
				struct Aluno *al_prox = al_atual->anterior;
				struct Aluno *al_atual_aux = al_prox;
				...
				if(al_atual_aux==NULL){
	printf("Todos registros Excluidos com sucesso !");}
				}
...
void exibir_maior_menor(){
	...
	while(al_atual_aux!=NULL){
		...
		printf("\nQuantidade de caracteres: %d",tamanho_maior);
	...
	while(al_atual_aux!=NULL){
		...
		printf("\nQuantidade de caracteres: %d",tamanho_menor);
